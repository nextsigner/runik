                                         ...
                                         double lon, double lat, double alt)
{
    ...
    // Asignación del estado de retrogradación como booleano puro
    obj["isRetro"] = (speedLong < 0.0);
    ...
    QString SwissEphManager::getHousesPos(int a, int m, int d, int h, int min, int gmt,
                                      double lat, double lon, int alt, QString hsys)
{
    ...
    // ¡CRUCIAL!: Si el sistema de casas es Topocéntrico ('T'),
    // le configuramos la ubicación exacta (incluyendo la altitud) a la librería.
    if (sys == 'T') {
        swe_set_topo(lon, lat, static_cast<double>(alt));
    }
    ...
    // Si sys es 'T', usará los datos geográficos que seteamos arriba con swe_set_topo
    ...
    for (int i = 1; i <= 12; ++i) {
        ...
        QString nomItem = "h" + QString::number(i);
        housesObj[nomItem] = houseData;
    }

    // 5. Agregar puntos principales
    root["ph"] = housesObj;
    root["asc"] = ascmc[0];
    root["mc"] = ascmc[1];
    root["vertex"] = ascmc[3];
    root["alt"] = alt;
    root["hsys"] = QString(sys); // Añadimos el sistema usado al JSON para control
